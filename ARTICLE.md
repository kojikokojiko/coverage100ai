...
私の意見としては、テストカバレッジ100%へのこだわりは開発スピードを鈍化させる原因になるし、                                                    
意味の薄いテストが量産されてCIリソースを無駄に消費するため、費用対効果に見合わないと思っている。                                              
                                                                                                                                            
とはいえ、どうしてもということなので割り切って対処することに、、
                                                                       
**Claude Code のカスタムスキル（スラッシュコマンド）** を作り、                                                                               
`/coverage <ファイルパス>` の一発で100%達成できる仕組みを構築した。  
